...
L1 = 20 #mm
L2 = 20 #mm
L3 = 20*math.sqrt(2) #mm
actualLength = [L1, L2, L3]
sorted_Actual_Length = []       
sortedIndices_Actual_Length = np.argsort(actualLength)
for ii in sortedIndices_Actual_Length:
    sorted_Actual_Length.append(actualLength[ii])
#~~~~~~~~~~~~~~~~~Getting images~~~~~~~~~~~~~~~~~
...
inputCaptureImage = input('Do you want to capture image? (y or yes)')
# Instantiate the stereoCameraControl Class
stereoCamera = stereoVision(Cam1ID=cv2.CAP_FIREWIRE+0, Cam2ID=cv2.CAP_FIREWIRE+1, setTrackbar='False', IsoSpeed=800, frameWidth=1024, frameHeight=768, target_directory = target_directory, mode = mode, CDM_MarkerNum = totalNumOfMarker, folderName_HSV_Value = 'HSV_Value', folderName_videoProperties = 'video_Capture_Properties')

if inputCaptureImage.lower() in ['yes', 'y']:
    stereoCamera.capture_image_press_key('Img 1','Img 2')


#~~~~~~~~~~~~~~~~~Centroid of marker extraction~~~~~~~~~~~~~~~~~
#imageFolderName = '2024-02-09_15-11-56'
stereoCameraAccuracyMeasurment_Name = []
os.chdir(target_directory + '/' + mode )
folderIndex = 0
for file in glob.glob('*'):
    folderIndex += 1
    stereoCameraAccuracyMeasurment_Name.append(file)
    print('folderIndex[',folderIndex,']:',file)

folderIndex = input('Enter the folder index: (ex: 1, 2, ...) ')
imageFolderName = stereoCameraAccuracyMeasurment_Name[int(folderIndex)-1]
...
#~~~~~~~~~~~~~~~~~Storing Centroid of marker in 2xN array~~~~~~~~~~~~~~~~~
